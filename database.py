...
class OfferDatabase:
    def __init__(self):
        ...
        try:
            ...
            # Sprawdź czy tabele zawierają dane
            cursor = self.conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM samochody")
            count = cursor.fetchone()[0]
            logger.info(f"Liczba rekordów w tabeli samochody: {count}")
            
            if count == 0:
                logger.warning("Brak danych w tabeli samochody - dodaję przykładowe dane")
